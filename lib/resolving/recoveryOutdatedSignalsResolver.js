...
RecoveryOutdatedSignalsResolver.prototype.resolveOutdatedSignals = function (trace, outdatedSignalIdentifiers) {
    ...
    outdatedSignalsToVisit.forEach(outdatedSignal => {
        if (outdatedSignal.sourceProcess !== undefined) {
            outdatedSignal.sourceProcess.outdatedInfo.addMissingOutput(outdatedSignal);
            outdatedSignal.sourceProcess.outdatedInfo.addFlag(OutdatedProcessInfo.prototype.FLAG_FORCE_RETRY);
            outdatedSignal.sourceProcess.outdatedInfo.addFlag(OutdatedProcessInfo.prototype.FLAG_OUTDATED_OUTPUT);
        }
    });
    ...
    while (outdatedSignalsToVisit.length > 0) {
        ...
        outdatedSignal.targetProcesses
            ...
            .forEach(targetProcess => {
                ...
                outdatedInfo.addFlag(OutdatedProcessInfo.prototype.FLAG_OUTDATED_OUTPUT);
