...
PersistenceLogInputOutput.prototype.getPersistenceFromJsonList = function (traceLogJsonList) {
    ...
    const rawInputs = this.filterPersistenceEntriesByType(traceLogJsonList, "input");
    const inputSignals = rawInputs.map(inputEntry => this.parseSignalPersistence(inputEntry["1"][2], undefined));
    inputSignals.forEach(inputSignal => firedSignalsMap[inputSignal.signalIdentifier] = inputSignal);
    ...
    return new ExecutionTrace(infos, rawInputs, firedProcesses, Object.values(firedSignalsMap), firedProcessIdentifierToOriginalMetadata);
};
...
PersistenceLogInputOutput.prototype.parseSignalPersistence = function (persistenceSignalEntry, sourceProcess) {
    ...
    const identifier = new SignalIdentifier(persistenceSignalEntry._id, persistenceSignalEntry.sigIdx, sourceProcessIdentifier, name);
    const workDir = persistenceSignalEntry.workdir;
    return new FiredSignal(name, identifier, [], sourceProcess, workDir);
};
...
PersistenceLogInputOutput.prototype.getResolvedTraceLogJsonList = function (resolvedExecutionTrace) {
    ...
    resolvedExecutionTrace.inputSignalsMetadata.forEach(input => jsonList.push(input));
    resolvedExecutionTrace.firedProcesses.map(process => {
        ...
        processMetadata["2"] = {
            ...
            "missingOutputs": Array.from(info.missingOutputs).map(signalIdentifier => signalIdentifier.id),
            "missingInputs": Array.from(info.missingInputs).map(signalIdentifier => signalIdentifier.id)
        };
